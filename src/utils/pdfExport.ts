...
export const generatePDF = async (
  ...
  doc.text(t.pdf.subtitle, pageWidth / 2, 25, { align: 'center' });
  ...
  doc.setFontSize(12);
  doc.setFont('helvetica', 'bold');
  doc.text(t.pdf.result, pageWidth / 2, y + 5, { align: 'center' });
  y += 15;

  doc.setTextColor(0, 0, 0);
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(t.pdf.currentRent, margin + 10, y);
  doc.setFont('helvetica', 'bold');
  doc.text(formatCurrency(formData.loyerMensuelActuel), pageWidth - margin - 10, y, { align: 'right' });
  y += 8;

  doc.setFont('helvetica', 'normal');
  doc.text(t.pdf.newRent, margin + 10, y);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(14);
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(t.pdf.variation, margin + 10, y);
  doc.setFont('helvetica', 'bold');
  ...
  doc.setFontSize(8);
  doc.setFont('helvetica', 'bold');
  doc.text(t.pdf.legalNotice, margin + 5, y + 8);
  
  doc.setFont('helvetica', 'normal');
  const avis = t.step5.legalNotice.text;
  ...
  doc.text(`${t.pdf.generatedOn} ${dateStr} ${t.pdf.by}`, pageWidth / 2, y, { align: 'center' });
